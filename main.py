...
# Entry windows are intentionally proportional to the selected timeframe so a
# 30-second setup does not leave an unnecessarily long entry period.
DEFAULT_ENTRY_SECONDS = max(1, int(os.getenv("ENTRY_SECONDS", "12")))
def entry_window_for_tf(tf):
    seconds = int(TIMEFRAMES.get(str(tf or "1m"), 60))
    # Roughly one third of a candle, capped at 15s. This keeps short-TF entries
    # tight while still giving longer timeframes a usable entry period.
    return max(2, min(DEFAULT_ENTRY_SECONDS, int(math.ceil(seconds / 3))))
